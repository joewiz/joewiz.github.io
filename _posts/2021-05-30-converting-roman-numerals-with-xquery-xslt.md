...
operation. I also tried using XQuery's 
[array](https://www.w3.org/TR/xquery-31/#id-arrays) 
facility and found it was a nice way to store and look up the tuple values for the fold 
function's accumulator, i.e., `[0, 0]` as the starting values and `[$running-total, 
$previous-number]` for ongoing values. Also, to achieve a similar clean, linear flow as the 
...
charaters by applying XQuery's powerful
[`analyze-string()`](https://www.w3.org/TR/xpath-functions-31/#func-analyze-string)
function.

Applying these techniques—folds, maps, the arrow operator, and the `for-each()` and
...
(Don't worry if you don't; I'll explain how below.) Here's the XQuery:
...
1. Moving through these integers from *right* to *left*, it compares each number to the previous value it had examined. (Upon first run, the starting "previous number" is simply 0.) If the current number is less than the previous value, it the current number subtracted from the running total, but otherwise, it is added to the running total. (Since the running total starts at 0, the first number is always added to 0, but any subsequent numbers will be added or subtracted accordingly.)
...
1. Proceeding from right-to-left, the first number checked is 1. Since 1 is greater than the default "previous number," 1 is added to the default "running total," yielding a value of 1. 
1. Continuing from right-to-left, the next number checked is 5. Since 5 is greater to the previous number, 1 is added to the default "running total," yielding a value of 6. 
1. With all numerals exhausted, the result of the function is 6.

The `fold-right()` function is responsible for the logic starting with line 4. Starting with a default "running total" of 0 and a default "previous number" of 0 (the `[0,0]` array), it begins running through each of the integers from right-to-left, one at a time. After each comparison, it updates the `$accumulator` variable with the new pair of values for the "running total" and "previous number." Once it's finished, we can access the final total by looking up the first value in the array returned by the `fold-right()` function.

A more complex example is "xliv", or 44:

1. The string is converted to "XLIV".
1. The uppercase Roman numeral is split into individual symbols: X, L, I, and V.
1. These symbols are converted into a sequence of 6 integers: 10, 50, 1, 5.
...
1. Continuing from right-to-left, the next number checked is 1. Since 1 is less than the previous number, 1 is subtracted from the running total, yielding a value of 4. 
1. The next number checked is 50. Since 50 is greater to the previous number, 50 is added to the running total, yielding a value of 54. 
1. The final number checked is 10. Since 10 is less than the previous number, 10 is added to the running total, yielding a value of 44. 
1. With all numerals exhausted, the result of the function is 44.
